...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const match = text.match(/ROOM\\.room_id\\s*=\\s*(\\d+);/);
    if (!match) return res.status(404).send('房间号错误');
    const realRid = match[1];

    // Extract obfuscated JS and evaluate it inside Node to get signed params.
    ...
    // ub98484234 returns a string containing inner function text; evaluate safely
    const vm = require('vm');
    ...
    try {
      vm.runInContext(func_sign + '\n;params = typeof sign === "function" ? sign("' + realRid + '","' + did + '","' + t10 + '") : null;', sandbox, { timeout: 2000 });
    } catch (e) {
      return res.status(500).send('生成参数失败');
    }
    const params = sandbox.params + `&cdn=ws-h5&rate=0`;

    const apiResp = await fetch(`https://playweb.douyu.com/lapi/live/getH5Play/${realRid}`, { method: 'POST', body: params, headers: { 'Content-Type': 'application/x-www-form-urlencoded' } });
